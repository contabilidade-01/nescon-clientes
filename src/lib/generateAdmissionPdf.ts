...
export async function downloadAdmissionPdf(p: Payload): Promise<void> {
  ...
  const sections: Array<{ title: string; rows: string[][] }> = [
    ...
    },
    {
      ...
      rows: [
        line("Livro / ficha de registro", simNao(d.docLivro)),
        line("CTPS", simNao(d.docCtps)),
        line("ASO admissional", simNao(d.docAso)),
        line("Foto 3x4", simNao(d.docFoto)),
        line("Cópias (CPF, RG, título, certidão, etc.)", simNao(d.docCopias)),
    ...
    },
    {
      ...
      rows: [
        ...
        line("Local / data emissão", `${d.identidadeLocal} ${d.identidadeEmissao}`.trim()),
        line("Telefone", d.telefone),
        line("CPF", d.cpf ? maskCPF(d.cpf) : ""),
        line("Título de eleitor", `${d.titulo}  Zona ${d.tituloZona}  Seção ${d.tituloSecao}  UF ${d.tituloUf}`),
